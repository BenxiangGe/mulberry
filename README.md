...
- Trait：支持 `trait`、concrete `impl Trait for Type` 和函数约束 `<T: Trait>`。当前真实
  协议是 `Show`；specialization 后静态选择普通函数，不生成 trait object 或 vtable。
...
- 控制流：`if` statement、exhaustive match statement/expression、terminal
  `yield` arm、`Result<T, E>` postfix `?`、`while`、`break` / `continue`、
  `for i in start .. end`。
...
- `safetensors`：使用 exact file I/O，并通过 `SafetensorsError` 组合 file、JSON、
  tensor lookup 和 metadata 错误的 open/find/read/write helper。
