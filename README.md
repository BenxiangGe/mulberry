...
位于 `packages/nn`；`stdlib/mulberry/nn.cherry` 只保留普通 extern package
...
`mulberry.nn` inference 示例可以直接用普通 `mulberry-driver file.cherry` 运行。
...
```sh
./build/release/bin/mulberry-driver --dump=mlir test/mulberry/Language/structs.cherry
./build/release/bin/mulberry-driver --dump=llvm test/mulberry/Driver/driver.cherry
./build/release/bin/mulberry-driver -c=/tmp/driver.o test/mulberry/Driver/driver.cherry
./build/release/bin/mulberry-driver -o /tmp/driver test/mulberry/Driver/driver.cherry
```
...
```sh
./build/release/bin/mulberry-driver -o /tmp/driver --bundle-runtime test/mulberry/Driver/driver.cherry
```
...
旧的 `cherry-driver`、`cherry-opt`、`cherry-translate` 和 `check-cherry`
兼容入口已经移除，命令统一使用 `mulberry-*`。
...
```sh
./build/release/bin/mulberry-driver examples/dl/inference_mnist_safetensors.cherry -o /tmp/mnist
/tmp/mnist
```
...
./build/release/bin/mulberry-driver --dump=mlir examples/dl/inference_mnist1.cherry
./build/release/bin/mulberry-driver --dump=lowered-mlir examples/dl/inference_mnist1.cherry
./build/release/bin/mulberry-driver examples/dl/inference_mnist1.cherry
...
- 源码文件扩展名仍然是 `.cherry`，后续是否改为 `.mulberry` 需要单独决定。
