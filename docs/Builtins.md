...
`Show.toString()` 提供 object identity default body；String 和 builtin scalar 用 concrete
`impl Show` 覆盖为 value formatter，user object 的 inherent `toString()` 优先。Sema 在
concrete type 上实例化 default body 或选择 concrete witness，最终都是普通 direct call，
