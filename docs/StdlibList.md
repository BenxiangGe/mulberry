...
```text
xs.size()       -> std.collections.ListStorage.size(xs)
xs.push(value)  -> std.collections.ListStorage.push(xs, value)
xs[index]       -> std.collections.ListStorage.get(xs, index)
xs[index] = v   -> std.collections.ListStorage.set(xs, index, v)
```
...
在当前 heap object handle 模型下，`List<T>` 仍然是 `Ptr<ListStorage<T>>`。method
可以先声明在 `ListStorage<T>` 内，receiver 显式写成 `Ptr<ListStorage<T>>`：
...
这样 `var xs: List<UInt64>` 的运行时语义仍然是一个 typed pointer，`xs.size()` 只是通过
`Ptr<ListStorage<UInt64>>` 找到 `ListStorage<UInt64>` 上的 method。
...
`size/push`
已经放进 `ListStorage<T>`，用户层写法是 `xs.size()`、`xs.push(value)`。
读取和写入元素使用下标语法：`xs[index]`、`xs[index] = value`。

语法层面的 `xs[i]` 已经在 Sema/MLIRGen 里走 `ListStorage.get`：
