...
- `List<T>` 定义 list header layout；可变共享语义通过 `Ptr<List<T>>` 表达。
...
这里 `List<T>` 只是 layout。需要可变 list object 时，源码层使用
`Ptr<List<T>>`，也就是“指向 list header 的 typed pointer”。`length/capacity/data`
都保存在同一个 heap object 里；`data` 指向 element buffer。复制 `Ptr<List<T>>`
会共享同一个 list object。
...
fn new<T>(): Ptr<List<T>>;
fn withCapacity<T>(capacity: UInt64): Ptr<List<T>>;

fn size<T>(xs: Ptr<List<T>>): UInt64;
fn get<T>(xs: Ptr<List<T>>, index: UInt64): T;
fn set<T>(xs: Ptr<List<T>>, index: UInt64, value: T): ();
fn push<T>(xs: Ptr<List<T>>, value: T): ();
fn pop<T>(xs: Ptr<List<T>>): T;
```
...
例如 `var xs: Ptr<collections.List<UInt64>> = collections.new();`。
...
```cherry
fn push<T>(xs: Ptr<List<T>>, value: T): UInt64 {
  ...
}
```
...
当前 `std.collections` 已经实现最小 `List<T>` layout，以及 `new<T>()`、
`withCapacity<T>()`、`size<T>()`、`get<T>()`、`set<T>()`、`push<T>()`。其中
`withCapacity<T>()` 会分配 list header 和 element buffer；`push<T>()` 当前只支持
容量足够的正向路径，还不做 grow。
