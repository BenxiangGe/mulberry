...
- 源语言 `List<T>` 已经走 stdlib/comptime 路径，语义上是普通 header struct；
  `length/capacity/data` 按 value 复制，`data` buffer 共享。
...
- `mulberry_core.ptr` / `mulberry_core.heap.alloc` / `mulberry_core.alloca` / `mulberry_core.load` /
  `mulberry_core.store` / `mulberry_core.ptr.index`：表达统一的 C/C++ typed pointer 模型。
...
`Ptr<T>`，由 `heap.alloc<T>(N)` 分配；复制、传参和返回都会复制 header，但共享同一个
元素 buffer。

这条路径刻意保持简单：

- array literal 分配 heap data buffer，然后构造 `{ length, data }` header。
- `array[i]` 通过 header.data 做 `ptr.index`，不再使用 `memref.load/store`。
- 函数参数和返回值传递 ordinary record value，因此语义是浅拷贝 header。
- `Array<T, N>` 可以进入 `struct` field、`Ptr<Array<...>>`、`heap.alloc<...>` 和
  nested Array。
...
`length` 是源码字符串的字节数，不包含结尾的 `\0`。`data` 指向字节数据。函数参数、
返回值和 record field 里的 `String` 都按普通 record value 传递；真正共享的是
`data` 指向的 byte buffer。
...
`handle` 保存 `fopen` 返回的 opaque C `FILE*`。`io.open` 调用 runtime wrapper
...
函数：它们取出 `File.handle` 和 `Tensor<T>.data`，把 data pointer 用 `ptr.asUInt8`
...
stdlib/comptime 定义成普通 header struct，函数参数和返回值按 ordinary record value
传递，不再需要单独的 list descriptor boundary pass。
...
- `Tensor<T>` 参数和返回值按普通 record header 传递。
...
```text
List<T> value
  -> mulberry_core.record value
  -> ordinary LLVM struct value

List<T> method receiver
  -> temporary Ptr<List<T>>
...
当前复杂对象统一倾向于 C/C++ 指针模型：`Ptr<T>` 是 typed pointer，heap storage
由 Boehm GC 管理。`List<T>` 本身是浅拷贝 header struct；返回或传参会复制
`length/capacity/data` 三个字段，多个 header 可以共享同一个 `data` buffer，不再需要
list-specific escape descriptor。
