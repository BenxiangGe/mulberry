...
2. `malloc(length * sizeof(element ABI))` 分配 caller-owned data buffer。
...
- 支持 element ABI 可 lower 的返回 list，包括当前的 scalar list 和
  `List<TensorDesc>`。
- lowering 使用 `malloc` 分配返回 descriptor 的 heap data。
- caller 侧在直接使用完返回 descriptor 后插入内部
  `mulberry.list.desc_dealloc`，lowering 后调用 `free(data)`。

本地 `List<T>` literal 如果只被直接 `list.get/list.size` 使用，`--lower-mulberry`
会在函数返回前插入内部 `mulberry.list.dealloc`。memref-backed list storage lowering
后变成 `memref.dealloc`。

这不是最终 ownership 方案，只是为了避免返回悬空 pointer，同时让当前正向路径可以
完整 lower。当前策略故意很窄：callee 不释放参数，callee 不释放返回 operand；
返回值 ownership 转移给 caller。
...
- 完整 ownership，包括任意控制流下的 dealloc、arena、GC、move/copy 语义。
...
R3.8.90 已完成：本地 list storage 支持内部显式 dealloc，`List<Tensor>` 返回
descriptor 的 heap data 支持 caller-side `free`。这只是当前正向子集，不是完整
runtime ownership。

R3.8.91 已完成：返回的 scalar `List<T>` 和 `List<Tensor>` 统一走 escaping
descriptor storage，`desc_pack` 会复制到 malloc-owned ABI data，caller 侧
`desc_dealloc` 会释放返回 descriptor 的 `data`。

R3.8.92：继续扩展 nested list、record field list 和最终 LLVM/JIT pipeline。
