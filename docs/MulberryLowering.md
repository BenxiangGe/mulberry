...
新的 heap object handle 方向混淆。当前 `List<Tensor>` boundary 的主路径是：

```text
list.desc_get -> tensor.desc_unpack -> Tensor/memref value
```

未来真正的 Tensor heap object handle 应该是 source-level record/Ptr 模型，等
generic struct、连续 heap storage 和 Tensor storage header 能力补齐后再实现。
