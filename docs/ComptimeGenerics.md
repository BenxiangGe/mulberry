...
就是这个 concrete struct type。MLIRGen 仍然只看到普通 `mulberry.record`，
不会把 generic 语义带进 IR。
...
`prelude.mulberry` 默认 import `collections.List` 和 `std.types`。用户代码直接使用裸
...
这里 `T` 是类型参数。`rank`、`sizes` 和 `strides` 都是 runtime metadata。现有
memref/linalg boundary 仍然需要 ranked view，所以 `tensor.view(value)` 需要从 expected
ranked Tensor type 或 NN op 语境里得到 memref rank。
