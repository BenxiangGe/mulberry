...
```mulberry
fn main(): UInt64 {
  const xs: types.Vector<UInt64> = list.from([1, 2, 3]);
  xs.size()
}
```
...
`List<UInt64>` 会在 Sema 阶段实例化成一个 concrete struct type，`List<UInt64>`
就是这个 concrete struct type。MLIRGen 仍然只看到普通 `mulberry_core.record`，
不会把 generic 语义带进 IR。
...
当前 generic struct alias 已经可以生成高层 MLIR/`mulberry_core.record`。Lowering/JIT
...
统一 `Ptr<T>` / heap storage 模型见 `docs/PtrAndHeapObjectModel.md`；
...
访问直接使用 `tensor[i]` / `tensor[i, j]`，由 MLIRGen 根据 `strides` 计算线性
offset。source-level ranked tensor bridge 已删除；后续 memref/linalg interop 应在
package/lowering 内部直接消费 `Tensor<T>` header。
