...
当前推荐使用 `Tensor<Float32, Rank>` handle；旧的 `Float32[?, ...]` Tensor value
expected type 仍保留，用于兼容现有 memref-backed Tensor 路径。
...
- lowering 使用实际 shape 分配动态 Tensor value 和 heap `TensorStorage` handle。
- runtime 把 payload bytes 读入 Tensor value，最终返回 `Tensor<Float32, 2>` handle。
...
这条旧 Tensor value 路径会额外检查 header 里的第一维必须是 `30`。等 source-level
Tensor handle 支持静态维度参数后，优先把这个能力迁到 `Tensor<..., ...>` 形式。
...
var w1: Tensor<Float32, 2> = readTensor(tensors, "w1");
```
...
- 如果 expected type 是 `Tensor<Float32, Rank>`，返回 heap Tensor handle。
- 如果 expected type 是旧 `Float32[?, ...]` Tensor value，直接返回 memref-backed
  Tensor value。
...
- `readTensor()` 可以返回 `Float32[?, ?]` 这类 Tensor value，也可以在 expected type
  为 `std.tensor.Tensor<Float32, Rank>` 时返回 heap Tensor handle。
