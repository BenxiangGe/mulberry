// RUN: cherry-opt --load-dialect-plugin=%cherry_libs/MulberryNNPackage%shlibext --load-pass-plugin=%cherry_libs/MulberryNNPackage%shlibext --pass-pipeline='builtin.module(lower-mulberry-nn)' %s | FileCheck %s --check-prefix=LOWER
// RUN: cherry-opt --load-dialect-plugin=%cherry_libs/MulberryNNPackage%shlibext --load-pass-plugin=%cherry_libs/MulberryNNPackage%shlibext --pass-pipeline='builtin.module(prepare-mulberry-nn-calls,lower-mulberry-nn)' %s | FileCheck %s --check-prefix=CALL
