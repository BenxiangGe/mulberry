...
auto getTensorDataAddressSpace(MLIRContext* context) -> Attribute {
  return LLVM::AddressSpaceAttr::get(context, 0);
}

auto convertTensorToDataMemRefType(TensorType type) -> MemRefType {
  auto layout = MemRefLayoutAttrInterface{};
  std::vector<int64_t> shape;
  for (auto dim : type.getShape())
    shape.push_back(dim < 0 ? ShapedType::kDynamic : dim);
  return MemRefType::get(shape, type.getElementType(), layout,
...
auto convertToTensorABILayout(MLIRContext* context, size_t rank)
    -> TensorABILayout {
...
auto convertToTensorABILayout(TensorType type) -> TensorABILayout {
  return convertToTensorABILayout(type.getContext(), type.getShape().size());
}

auto createMemRefDataPointer(Location location, OpBuilder& rewriter,
                             Value storage) -> Value {
...
auto createTensorByteSize(Location location,
...
auto createTensorStorageOwner(Location location, OpBuilder& builder,
...
auto extractRecordField(Location location,
                        ConversionPatternRewriter& rewriter, Value record,
                        RecordType recordType, llvm::StringRef field)
    -> FailureOr<Value> {
...
auto getReferencedRecordType(Type type) -> RecordType {
...
auto createRecordFieldAddress(Location location,
                              ConversionPatternRewriter& rewriter,
                              Value record, RecordType recordType,
                              llvm::StringRef field) -> FailureOr<Value> {
...
auto loadTensorStorageReference(Location location,
                                ConversionPatternRewriter& rewriter,
                                Value tensor, RecordType tensorType)
    -> FailureOr<Value> {
...
auto getTensorStorageType(RecordType tensorType) -> RecordType {
  return getReferencedRecordType(tensorType.getFieldType("_storage"));
}

auto assertTensorStorageAlive(Location location,
                              ConversionPatternRewriter& rewriter,
                              Operation* op, Value storage,
                              RecordType storageType) -> LogicalResult {
...
auto disposeTensorStorage(Location location,
                          ConversionPatternRewriter& rewriter, Operation* op,
                          Value storage, RecordType storageType)
    -> LogicalResult {
...
auto loadRecordValue(Location location, ConversionPatternRewriter& rewriter,
                     Value recordReference, RecordType recordType)
    -> FailureOr<Value> {
...
auto loadI64PointerElement(Location location,
                           ConversionPatternRewriter& rewriter, Value ptr,
                           size_t index) -> Value {
...
void storePointerElement(Location location, ConversionPatternRewriter& rewriter,
...
auto createHeapArray(Location location, ConversionPatternRewriter& rewriter,
                     Operation* op, Type elementType, Value count)
    -> FailureOr<Value> {
...
auto createListRecordValue(Location location,
                           ConversionPatternRewriter& rewriter,
                           RecordType listType, Value length, Value data)
    -> FailureOr<Value> {
...
auto createHeapRecordReference(Location location,
                               ConversionPatternRewriter& rewriter,
                               Operation* op, RecordType recordType,
...
auto extractListDataPointer(Location location,
                            ConversionPatternRewriter& rewriter, Value list,
                            RecordType listType) -> FailureOr<Value> {
...
auto createTensorABIDesc(Location location,
                         ConversionPatternRewriter& rewriter,
                         const TensorABILayout& layout, Value tensor,
...
auto createTensorABIDescFromRecord(
    Location location, ConversionPatternRewriter& rewriter,
    const TensorABILayout& layout, Value tensorRecord,
    RecordType tensorRecordType, TensorType tensorType) -> FailureOr<Value> {
...
auto createScalarMemRefFromDataPointer(Location location,
                                       ConversionPatternRewriter& rewriter,
                                       MemRefType memRefType, Value dataPointer)
    -> Value {
  auto context = rewriter.getContext();
  auto i64Type = getI64Type(context);
...
auto createTensorFromABIDesc(Location location,
                             ConversionPatternRewriter& rewriter, Value desc,
                             TensorType type) -> Value {
  auto context = rewriter.getContext();
...
auto createTensorFromFlatStorage(Location location,
                                 ConversionPatternRewriter& rewriter,
                                 Value storage, Value desc, TensorType type)
    -> Value {
  auto context = rewriter.getContext();
  auto resultMemRefType = convertTensorToMemRefType(type);
...
public:
  ...
  auto matchAndRewrite(TensorPackOp op, OpAdaptor adaptor,
    ...
    auto tensorType = llvm::cast<TensorType>(op.getTensor().getType());
...
public:
  ...
  auto matchAndRewrite(TensorAssertAliveOp op, OpAdaptor adaptor,
                       ConversionPatternRewriter& rewriter) const
      -> LogicalResult final {
    auto tensorPtrType = llvm::dyn_cast<PtrType>(
        op.getTensor().getType());
...
void populateFinalizeMulberryTensorStoragePatterns(RewritePatternSet& patterns,
                                                   MLIRContext* context) {
  patterns.add<FinalizeTensorStorageAllocPattern>(context);
}
